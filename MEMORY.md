...
- 公考信息采集完成(成都3条)，仍为2024年历史数据
- QMD知识库：64文件/486块/486 FTS，向量索引仍为0（内存受限）
- 内存使用1.0G/1.9G，CPU负载正常(0.59)，系统健康
- ⚠️ 石油黄金 cron 推送任务（早盘/日盘/晚盘/美盘）仍未恢复，OpenClaw cron 列表中已消失
- ⚠️ 晚间回顾 cron 也已不在 OpenClaw cron 列表中
- 周日，官家可能在休息，无用户主动交互
- 系统 crontab 中的石油黄金脚本仍在运行（每小时7d分析+每日日报+每周一90d分析）
...
- **状态:** 64文件/486块/486 FTS；向量索引受限于内存（1.9G），Qwen3-Embedding-0.6B 加载时 OOM
