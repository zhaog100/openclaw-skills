...
- ⚠️ 石油黄金 cron 推送任务（早盘/日盘/晚盘/美盘）仍未恢复 OpenClaw cron，但通过系统 cron 正常运行
- ⚠️ 石油黄金报告每次都被重复推送（每个时段×2次），累计8次推送，需排查
- 周日，官家可能在休息，无用户主动交互
- 系统 crontab 全部正常（9个任务），OpenClaw cron 仅存晚间回顾
- 晚间回顾 cron 恢复（22:50 正常触发执行）
...
- [ ] ⚠️ 石油黄金报告重复推送（6月6日和7日都出现，每个时段×2次），需排查 cron 重复触发原因
- [ ] 公考信息采集脚本返回2024年历史数据，数据源需检查
