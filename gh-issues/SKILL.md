...
name: gh-issues
version: 1.0.0
description: "Fetch GitHub issues, spawn sub-agents to implement fixes and open PRs, then monitor and address PR review comments. Usage: /gh-issues [owner/repo] [--label bug] [--limit 5] [--milestone v1.0] [--assignee @me] [--fork user/repo] [--watch] [--interval 5] [--reviews-only] [--cron] [--dry-run] [--model agnes-2.5-flash] [--notify-channel -1002381931352]"
user-invocable: true
metadata:
  {
    "openclaw":
...
| --model | _(none)_ | Model to use for sub-agents . If not specified, uses the agent's default model. |
