...
**核心需求**：设计双 OpenClaw 协作系统，让两个智能体（小米粒 PM + 小米粒 Dev）能够：
- ✅ 各司其职（创意策划 vs 执行落地）
- ✅ 无缝衔接（B 能读到 A 的产出）
- ✅ 24/7 高效协作（无需人工复制粘贴）
- ✅ 跨环境部署（家里 NAS + 公司服务器）
...
#### 写回机制
1. 小米粒的产出（PRD/Review）自动写回 MemOS
...
- **描述**：为两个实例配置不同的角色和技能
- **配置文件**：
  - 实例 A：`~/.openclaw-home/openclaw.json`（小米粒 - PM）
...
1. 官家提出需求
   ↓
2. 小米粒（PM）分析需求 → 创建 PRD → 写入 MemOS
   ↓
3. 小米粒（Dev）检索 MemOS → 读取 PRD → 技术设计
   ↓
4. 小米粒 开发实现 → 自测 → 提交 Review 请求
   ↓
5. 小米粒 检索 MemOS → 读取设计/代码 → Review
   ↓
