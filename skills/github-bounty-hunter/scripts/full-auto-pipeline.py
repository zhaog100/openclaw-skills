...
#!/usr/bin/env python3
"""
GitHub Bounty Hunter - 全自动流水线 v7.5.6
...
# ── 仓库黑名单 ──
BLACKLIST_REPOS = [
    os.environ.get('GITHUB_USERNAME', 'your_username'), 'Scottcjn', 'rustchain', 'solfoundry', 'aporthq', 'rohitdash08',
...
- 只返回代码，用 ```语言 包裹，不要解释"""
    ...
    fallback_models = [
        os.getenv('AI_MODEL', 'agnes-2.0-flash'),
        'agnes-2.0-flash',
...
# Task: {title}
# Repository: {repo}
# Generated by GitHub Bounty Hunter v7.5.6
...
def has_existing_pr(task):
    ...
    try:
        r = requests.get(
            ...
            headers={'Authorization': f'token {GITHUB_TOKEN}'},
            params={'state': 'open', 'head': f'{os.environ.get('GITHUB_USERNAME', 'your_username')}:bounty-{number}'},
            timeout=10
        )
...
def fork_repo(owner, repo):
    """Fork仓库到your_username，返回fork的URL"""
    fork_url = f"https://api.github.com/repos/{owner}/{repo}/forks"
    headers = {'Authorization': f'token {GITHUB_TOKEN}'}
    
    # 先检查是否已fork
    check = subprocess.run(
        ['gh', 'repo', 'view', f'{os.environ.get('GITHUB_USERNAME', 'your_username')}/{repo}', '--json', 'url'],
        capture_output=True, text=True
    )
    if check.returncode == 0:
        log(f"✅ 仓库已fork：{os.environ.get('GITHUB_USERNAME', 'your_username')}/{repo}")
        return True
    
    log(f"🍴 Fork仓库：{owner}/{repo} → {os.environ.get('GITHUB_USERNAME', 'your_username')}/{repo}")
...
def commit_and_push(project_dir, task, fork_owner=os.environ.get('GITHUB_USERNAME', 'your_username')):
    """提交并推送到fork仓库"""
    number = task.get('number')
    title = task.get('title', '')
...
def create_pull_request(task):
    ...
    log(f"🔗 创建 PR：{owner}/{repo} ← your_username/{repo}")
...
*Auto-submitted for review* 🚀"""
    ...
    data = {
        'title': pr_title,
        'body': pr_body,
        'head': f"your_username:{branch_name}",
...
def acquire_lock():
    """文件锁，防止cron并发"""
    import tempfile; lock_file = open(Path(tempfile.gettempdir()) / 'bounty-pipeline.lock', 'w')
    try:
