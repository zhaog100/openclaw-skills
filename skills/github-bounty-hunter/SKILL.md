...
description: GitHub 赏金猎人。自动监控 GitHub 上的 grant/bounty 项目，支持 Algora 平台，使用 /attempt 命令自动接任务、开发、提交 PR，让 OpenClaw 自己赚钱！v6.1 新增双重过滤、2026-04-09 实战验证、RTC 任务专项支持
version: 6.1.1
author: 小米辣 🌶️
---
...
**核心原则：自动扫描 → 自动认领 → 自动开发 → 自动提交 → 继续下一个**

### 执行规则
1. **永不停止** — 只要发现高质量任务就继续，完成后汇报，不问"继续吗？"
2. **无需确认** — 不问"开始吗？"、"做这个吗？"，直接执行
3. **按顺序完成** — 一个接一个，中间不暂停
4. **质量优先** — 只做评分 ≥ 50 的高质量任务（USD ≥ $100 或 RTC ≥ 10）
   - 评分算法: `score = (amount * 0.5) + (100 - comments) * 0.3 + (tech_match * 0.2)`
   - score > 70 → P0 立即执行
   - score > 50 → P1 马上执行
   - score ≤ 50 → 跳过，不做低质量任务
5. **智能跳过** — 遇到问题自动跳过或重试，不中断流程

### 执行流程
```
扫描 → 过滤(≥$100 / ≥10 RTC) → 预检(归档/活跃度/已有PR) → 认领 → 开发 → 测试 → 提交 → 下一个
```
...
| 指标 | 无策略 | v5.2 | v6.0 (今日) | 提升 |
|------|--------|------|-------------|------|
| 接单成功率 | 30% | 75% | 100% | +233% |
| 时间利用率 | 40% | 85% | 95% | +137% |
| 平均收益/小时 | $50 | $120 | $150 | +200% |
| 无效开发率 | 60% | 10% | 0% | -100% |
| 低价值任务占比 | 50% | 20% | 0% | -100% |
