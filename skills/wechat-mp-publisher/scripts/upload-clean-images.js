/**
 * 上传无方框的简洁图片到微信服务器并发布文章
 */

const { chromium } = require('playwright');
const fs = require('fs');

async function uploadCleanImagesAndPublish() {
  console.log('🖼️ 开始上传无方框的简洁图片到微信服务器...');
  ...
  try {
    ...
    // 定义所有图片（无方框设计）
    const images = [
      { name: 'cover', title: '缓解焦虑的 5 个呼吸法', subtitle: '心灵的寻光之旅', emoji: '🧘‍♀️', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
      { name: 'intro', title: '为什么呼吸能缓解焦虑？', subtitle: '呼吸是连接身体与心灵的桥梁', emoji: '🌊', gradient: 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)' },
      { name: 'breath1', title: '1️⃣ 腹式呼吸', subtitle: '基础版 | 4秒吸气 → 6秒呼气', emoji: '🫁', gradient: 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)' },
      { name: 'breath2', title: '2️⃣ 4-7-8 呼吸法', subtitle: '睡前最佳 | 4秒→7秒→8秒', emoji: '🌙', gradient: 'linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%)' },
      { name: 'breath3', title: '3️⃣ 交替鼻孔呼吸', subtitle: '平衡左右脑 | 5-10轮', emoji: '🔄', gradient: 'linear-gradient(135deg, #e0c3fc 0%, #8c52ff 100%)' },
      { name: 'breath4', title: '4️⃣ 箱式呼吸', subtitle: '工作间隙 | 4秒→4秒→4秒→4秒', emoji: '⬜', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
      { name: 'breath5', title: '5️⃣ 蜂鸣呼吸', subtitle: '释放紧张 | 嗡嗡振动', emoji: '🐝', gradient: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' },
      { name: 'conclusion', title: '💝 结语', subtitle: '每一次深呼吸都是对自己的温柔拥抱', emoji: '💝', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }
    ];
    ...
    for (const img of images) {
      ...
      // 截图
      const imagePath = `/tmp/wechat-clean-${img.name}.png`;
      await page.screenshot({ path: imagePath, fullPage: true });
      
      // 上传到微信服务器
      console.log(`📤 上传图片：${img.name}`);
      const uploadUrl = `https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=${accessToken}&type=image`;
      
      const imageBuffer = fs.readFileSync(imagePath);
      const formData = new FormData();
      formData.append('media', new Blob([imageBuffer]), `clean-${img.name}.png`);
      ...
      if (uploadData.url) {
        console.log(`✅ 图片上传成功：${img.name} → ${uploadData.url}`);
        uploadedImages.push({
          name: img.name,
          url: uploadData.url,
        ...
        console.error(`❌ 图片上传失败：${img.name} → ${JSON.stringify(uploadData)}`);
      }
    }
    ...
    const articleData = {
      articles: [{
        ...
        content: `
          ...
          <img src="${uploadedImages.find(img => img.name === 'intro')?.url || ''}" alt="为什么呼吸能缓解焦虑" style="width: 100%; border-radius: 8px; margin: 20px 0;" />
          ...
          <div style="background: linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%); padding: 25px; border-radius: 12px; margin: 20px 0;">
            ...
            <img src="${uploadedImages.find(img => img.name === 'breath2')?.url || ''}" alt="4-7-8 呼吸法" style="width: 100%; border-radius: 8px; margin: 15px 0;" />
          </div>
          ...
          <div style="background: linear-gradient(135deg, #e0c3fc 0%, #8c52ff 100%); padding: 25px; border-radius: 12px; margin: 20px 0;">
            ...
            <img src="${uploadedImages.find(img => img.name === 'breath3')?.url || ''}" alt="交替鼻孔呼吸" style="width: 100%; border-radius: 8px; margin: 15px 0;" />
          </div>
          ...
          <img src="${uploadedImages.find(img => img.name === 'conclusion')?.url || ''}" alt="结语" style="width: 100%; border-radius: 8px; margin: 20px 0;" />
    ...
    if (draftData.media_id) {
      ...
      console.error(`❌ 文章发布失败: ${JSON.stringify(draftData)}`);
      return { 
        success: false, 
        message: `文章发布失败: ${JSON.stringify(draftData)}` 
      };
    }
  } catch (error) {
...
uploadCleanImagesAndPublish().then(result => {
