...
async function uploadRealPhotosAndPublish() {
  console.log('📸 开始下载真实照片并上传到微信服务器...');
  ...
  try {
    ...
    // 真实照片 URL（Pexels 瑜伽/冥想/呼吸主题）
    const photoUrls = {
      cover: 'https://images.pexels.com/photos/3829109/pexels-photo-3829109.jpeg?auto=compress&cs=tinysrgb&w=800',
      intro: 'https://images.pexels.com/photos/3822722/pexels-photo-3822722.jpeg?auto=compress&cs=tinysrgb&w=800',
      breath1: 'https://images.pexels.com/photos/3829109/pexels-photo-3829109.jpeg?auto=compress&cs=tinysrgb&w=800',
      breath2: 'https://images.pexels.com/photos/3822722/pexels-photo-3822722.jpeg?auto=compress&cs=tinysrgb&w=800',
      breath3: 'https://images.pexels.com/photos/3829109/pexels-photo-3829109.jpeg?auto=compress&cs=tinysrgb&w=800',
      breath4: 'https://images.pexels.com/photos/3822722/pexels-photo-3822722.jpeg?auto=compress&cs=tinysrgb&w=800',
      breath5: 'https://images.pexels.com/photos/3829109/pexels-photo-3829109.jpeg?auto=compress&cs=tinysrgb&w=800',
      conclusion: 'https://images.pexels.com/photos/3822722/pexels-photo-3822722.jpeg?auto=compress&cs=tinysrgb&w=800'
    };
    ...
    for (const [name, url] of Object.entries(photoUrls)) {
      console.log(`📸 下载照片：${name}`);
      const imagePath = `/tmp/wechat-real-${name}.jpg`;
      
      try {
        await downloadImage(url, imagePath);
        console.log(`✅ 照片下载成功：${name}`);
        
        // 上传到微信服务器
        console.log(`📤 上传照片：${name}`);
        const uploadUrl = `https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=${accessToken}&type=image`;
        
        const imageBuffer = fs.readFileSync(imagePath);
        const formData = new FormData();
        formData.append('media', new Blob([imageBuffer]), `real-${name}.jpg`);
        ...
        if (uploadData.url) {
          console.log(`✅ 照片上传成功：${name} → ${uploadData.url}`);
          uploadedImages.push({
            name: name,
            url: uploadData.url,
          ...
          console.error(`❌ 照片上传失败：${name} → ${JSON.stringify(uploadData)}`);
        }
      } catch (error) {
        console.error(`❌ 照片处理失败：${name} → ${error.message}`);
      }
    }
    
    console.log(`\n✅ 共上传 ${uploadedImages.length} 张照片`);
...
uploadRealPhotosAndPublish().then(result => {
