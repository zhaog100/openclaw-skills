...
| **Coding (代码)** | `.js`, `.jsx`, `.ts`, `.tsx`, `.py`, `.java`, `.cpp`, `.c`, `.html`, `.css`, `.json`, `.xml`, `.yaml`, `.yml` | `zai/glm-5.1` | - | 代码分析/生成 |
| **Complex (文档)** | `.pdf`, `.doc`, `.docx`, `.ppt`, `.pptx`, `.xls`, `.xlsx` | `longcat/LongCat-Flash-Thinking-2601` | - | 文档解析/深度分析 |
| **Main (常规)** | `.txt`, `.md`, `.csv` | `longcat/LongCat-Flash-Chat` | - | 普通文本处理 |
...
|---------|--------|--------|---------|---------|
| **Flash** | `longcat/LongCat-Flash-Lite` | LongCat | 5500 万 token/天 | 简单问答、快速查询 |
| **Main** | `longcat/LongCat-Flash-Chat` | LongCat | 500 万 token/天 | 常规对话、分析任务 |
| **Coding** | `zai/glm-5.1` | 智谱 | 付费 | 代码生成/调试 |
...
| **Long-Context** | `openrouter/qwen3.6-plus-preview:free` | OpenRouter | 免费 | 100 万 token 上下文 |
...
# 输出示例:
{
  ...
  "selectedModel": "longcat/LongCat-Flash-Lite",
...
# [INFO] 关键词 "分析" → +1 分
# [RESULT] 最终选择：longcat/LongCat-Flash-Omni-2603
```
...
```bash
# 扫描任务 → 用 turbo (省钱)
sessions_spawn --model zai/glm-5-turbo --task "scan 所有日志文件"

# 编码任务 → 用 glm-5 (质量)
sessions_spawn --model zai/glm-5 --task "develop 一个 API 接口"

# 架构设计 → 用 glm-5 + thinking
sessions_spawn --model zai/glm-5 --thinking --task "design 系统架构"
```
...
| 任务类型 | 推荐模型 | 成本 | 理由 |
|---------|---------|------|------|
| 扫描/搜索 | `glm-5-turbo` | 低 | 高吞吐，快速完成 |
| 简单 Review | `glm-5-turbo` | 低 | 够用就好 |
| 编码/修复 | `glm-5` | 中 | 减少 bug，避免返工 |
| 架构设计 | `glm-5 + thinking` | 高 | 深度思考，质量优先 |
