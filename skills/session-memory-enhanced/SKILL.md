...
# 自动模式（crontab）
0 * * * * $(pwd)/skills/session-memory-enhanced/scripts/session-memory-enhanced.sh

# 手动执行
bash scripts/session-memory-enhanced.sh
...
├── scripts/session-memory-enhanced.sh  # 主脚本
...
### MEMORY-LITE.md 维护（v4.2.1 更新）
- `MEMORY-LITE.md` 是唯一允许全量读取的记忆文件
- 必须控制在 **5KB 以内**（当前 ~2.2KB）
- 由定时脚本自动从 MEMORY.md 精华摘要生成
- 内容：核心偏好、关键决策、重要约束
- **flush 去重**：写入前检查是否已有相同 timestamp 条目，避免重复
