...
def run_advisor_akshare(days=3):
    ...
    for name, r in results.items():
        ...
        if score >= 70:
            ...
            advice = "🔴 偏空（考虑做空或回避）"
        print(f"\n  🎯 技术面评分: {score}/100 → {advice}")

    # ━━━ 收集技术面评分 ━━━
    tech_scores = {}
    for name, r in results.items():
        tech_scores[name] = r["score"]
...
def _print_final_recommendation(results, tech_scores, risk_score, macro_data):
    """
    最终买卖建议：技术面 + 宏观面 + 地缘风险 = 综合决策
    
    评分体系:
      技术面权重 40% | 宏观面权重 35% | 地缘风险 15% | 市场情绪 10%
    ...
    print(f"{'━' * 50}")
    print(f"  综合以下维度：")
    print(f"    1️⃣ 技术面分析（RSI/MACD/KDJ/布林带/量价）")
    print(f"    2️⃣ 美国宏观数据（FRED 12项指标）")
    print(f"    3️⃣ 市场情绪（VIX/信用利差/消费者信心）")
    print(f"    4️⃣ 地缘政治风险")
    print(f"    5️⃣ 黄金/原油宏观信号灯")
    ...
    # 地缘风险评分转换：risk_score 0-50 → geo_impact 0-100
    # risk_score 低=平静，高=风险
    # 对黄金：风险高→利多（避险）→ 加分
    # 对原油：风险高→利空（需求不确定）→ 减分
    geo_base = risk_score if risk_score else 10  # 0-50 范围
    ...
    for name, r in results.items():
        ...
        # 判断品种类型
        is_gold = "黄金" in name or "gold" in name.lower() or "au" in name.lower()
        is_oil = "原油" in name or "oil" in name.lower() or "sc" in name.lower()
        ...
        # 地缘影响：转为0-100分数
        # 黄金：地缘风险→避险利多→加分 (geo_base * 2 映射到0-100)
        # 原油：地缘风险→需求不确定→减分
        if is_gold:
            geo_adj = 50 + geo_base  # 50(平静)+risk(0-50) = 50-100
        elif is_oil:
            geo_adj = 50 - geo_base * 0.5  # 50-risk/2 = 25-50
        else:
            geo_adj = 50
        ...
        try:
            ...
            pass
        sentiment_score = max(0, 100 - vix_val * 2.5)  # VIX<20=高分, VIX>40=低分
        ...
        print(f"\n  {'─' * 46}")
        print(f"  📊 {name} ({sym}{r['latest']})")
        print(f"  {'─' * 46}")
        print(f"    技术面: {tech}/100 ({'偏多' if tech > 60 else '偏空' if tech < 40 else '中性'})")
        print(f"    宏观面: {macro_combined:.0f}/100 ({'偏多' if macro_combined > 60 else '偏空' if macro_combined < 40 else '中性'})")
        print(f"    信号灯: {macro_signal:+d} ({'利多' if macro_signal > 0 else '利空' if macro_signal < 0 else '中性'})")
        print(f"    地缘风险: {geo_base}/50 ({'高' if geo_base > 30 else '中' if geo_base > 15 else '低'})")
        print(f"    ────────────────────────────────")
        print(f"    🎯 最终评分: {final_score:.0f}/100")
        print(f"    🚦 投资建议: {grade}")
        print(f"    📌 操作指引: {action}")
        ...
        if final_score >= 60 and r.get("atr"):
            ...
            if isinstance(atr, (int, float)) and atr > 0:
                # 短线/中线/长线 三级止损
                sl_short = price - 1.0 * atr   # 短线：1倍ATR
                sl_mid = price - 1.5 * atr     # 中线：1.5倍ATR
                tp1 = price + 2.0 * atr
                tp2 = price + 3.0 * atr
                print(f"    🛡️ 止损位:")
                print(f"       短线: {sym}{sl_short:.2f} (-{1.0*atr/price*100:.1f}%)")
                print(f"       中线: {sym}{sl_mid:.2f} (-{1.5*atr/price*100:.1f}%)")
                print(f"    🎯 目标位:")
                print(f"       目标1: {sym}{tp1:.2f} (+{2.0*atr/price*100:.1f}%)")
                print(f"       目标2: {sym}{tp2:.2f} (+{3.0*atr/price*100:.1f}%)")
                rr1 = (tp1 - price) / (price - sl_short) if price > sl_short else 0
                print(f"    📐 风险回报比: 1:{rr1:.1f} (短线) ~ 1:{(tp2-price)/(price-sl_mid):.1f} (中线)")
        elif final_score < 40:
            print(f"    💡 如已持仓，建议止损位: {sym}{r['latest'] * 0.97:.2f} (-3%)")
    ...
    risks = []
    if risk_score > 40:
        risks.append("地缘风险升高，市场波动可能加大")
    
    cs = macro_data.get("indicators", {}).get("UMCSENT")
    if cs and cs["value"] < 65:
        risks.append(f"消费者信心仅{cs['value']:.0f}，经济衰退风险")
    
    vix = macro_data.get("sentiment", {}).get("VIXCLS")
    if vix and vix["value"] > 25:
        risks.append(f"VIX={vix['value']:.0f}，市场恐慌中")
    
    real_rate = macro_data.get("indicators", {}).get("DFII10")
    if real_rate and real_rate["value"] > 2:
        risks.append(f"实际利率{real_rate['value']:.1f}%偏高，压制黄金")
