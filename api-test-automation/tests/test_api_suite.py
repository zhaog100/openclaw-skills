...
from reporter import TestReporter as _RealTestReporter, TestReport as _RealTestReport, TestResult as _RealTestResult

# Save references BEFORE the TestReporter test class is defined in this module,
# because pytest will overwrite the name 'TestReporter' with the test class itself.
RealTestReporter = _RealTestReporter
RealTestReport = _RealTestReport
RealTestResult = _RealTestResult
...
class TestMockServer:
    ...
    def test_add_route(self):
        """Test adding routes."""
        server = MockServer()
        route = MockRoute().with_method("GET").with_path("/test").with_response(200, {"test": True})
    ...
    def test_route_matching(self):
        """Test route matching."""
        route = MockRoute().with_method("GET").with_path("/users")
...
class TestReporter:
    ...
    def test_initialization(self, tmp_path):
        ...
        reporter = RealTestReporter(output_dir=str(tmp_path))
    ...
    def test_generate_html_report(self, tmp_path):
        ...
        results = [
            RealTestResult(name="test1", status="passed", duration=0.1),
            RealTestResult(name="test2", status="failed", duration=0.2, message="Error"),
        ]
        report = RealTestReport(timestamp=datetime.now(), results=results, total_duration=0.3)
    ...
    def test_generate_json_report(self, tmp_path):
        ...
        report = RealTestReport(timestamp=datetime.now(), results=results)
...
class TestTestReport:
    ...
    def test_report_calculations(self):
        ...
        results = [
            RealTestResult(name="t1", status="passed"),
            RealTestResult(name="t2", status="passed"),
            RealTestResult(name="t3", status="failed"),
            RealTestResult(name="t4", status="skipped"),
        ]
        report = RealTestReport(timestamp=datetime.now(), results=results)
