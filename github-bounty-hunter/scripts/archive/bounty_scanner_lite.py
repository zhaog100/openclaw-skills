...
BLACKLIST = [
    'your_username', 'Scottcjn', 'rustchain', 'solfoundry', 'aporthq', 
...
def has_existing_pr(task):
    ...
    try:
        r = requests.get(f'https://api.github.com/repos/{owner}/{repo}/pulls',
            headers=HEADERS, params={'state': 'open', 'head': f'your_username:bounty-{number}'}, timeout=10)
...
def clone_repo(owner, repo, number):
    ...
    fork_url = f"https://{GITHUB_TOKEN}@github.com/your_username/{repo}.git"
