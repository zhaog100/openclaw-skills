...
description: "GitHub 赏金猎人。自动监控 GitHub bounty，支持 Algora/UbiquityOS/RustChain 平台。v8.0 新增 RustChain 付款确认 + secondary rate limit 策略（2026-08-03）"
version: 8.0.0
author: 思捷娅科技 (SJYKJ)/zhaog100
---

# GitHub Bounty Hunter v8.0.0
...
- [🔧 环境变量](#-环境变量)
- [🚀 全自动执行策略](#-全自动执行策略)
- [📊 历史统计 vs 预期收益](#-历史统计-vs-预期收益)
- [🎯 目标平台](#-目标平台)
- [⚡ 5条强制规则](#-5条强制规则)
- [⚠️ 防屏蔽规则](#-防屏蔽规则)
- [⚠️ Claim Issue 格式规范](#-claim-issue-格式规范)
- [🧠 经验教训库](#-经验教训库)
- [🤖 v7.0 精耕细作策略](#-v70-精耕细作策略)
- [📄 许可证与版权声明](#-许可证与版权声明)
<!-- TOC -->

**v8.0 新增**：RustChain 付款确认（#8022 35 RTC fully paid）+ secondary rate limit 处理策略（2026-08-03）
**v7.5.6 新增**：180 RTC 最终催款经验 + 钱包余额归零预警（2026-06-10）
**v8.0 新增**：RustChain 付款确认案例 + secondary rate limit 处理

| PR | 仓库 | 金额 | 状态 | 备注 |
|----|------|------|------|------|
| #8022 | Scottcjn/Rustchain | 35 RTC | MERGED + PAID ✅ | 2026-07-27 合并，全额到账 |

**案例要点**：
- PR #8022 由 `dtopenclaw` 提交，非 `zhaog100`
- 合并日期：2026-07-27 17:32 UTC
- 付款时间：20 RTC (7/26) + 15 RTC (7/27)
- 确认 RustChain 有付款记录，可继续参与

**RustChain 优先级调整**：P3❌ → P2（有付款历史）

**secondary rate limit 处理**：
- 现象：HTTP 403 + `retry-after: 10` 头
- 原因：GitHub 二次限速（非正常 API 限流，频率限制触发后触发）
- 处理：遇到 403 等待 10-15 分钟再继续
- 避免：控制扫描频率（每 2 小时一次），单次扫描不超过 10 页
...
> ⚠️ **重要说明**：以下「预期收益」表是**历史统计推算**，仅供参考。实际到账请以各平台公示为准。

### 实际数据（截至 2026-04-12）
- **历史统计**：393 PR → 18 merged → $0 到账（广撒网时期）
- **v7.0 精耕细作**：10 PR → 部分 merged → 实际收益待验证（见下方「2026-04-12 实战数据」）
...
| Algora | P1 | 有托管机制 | 待验证 |
| RustChain | P2 ✅ | 19 merged 35 RTC paid | 35 RTC (#8022) |
...
### 规则 6：钱包余额验证（2026-06-08 新增）⭐⭐⭐⭐⭐

> **来源**: 2026-06-08 付款调查经验总结
> **重要教训**: Scottcjn 付款验证 - 确认 292 RTC 已实际到账
...
2. 有至少 3 笔付款记录
3. 最近 90 天内有付款活动
4. 交易来源包含 "founder_team_bounty" 或 "founder_dev_fund"
5. 金额分布合理（不是只有 0.1 RTC）
...
- **钱包名**: `zhaog100`
- **总余额**: 292.0 RTC（约 $29.20 USD）
- **交易笔数**: 9 笔
- **关键发现**: 
  - 2026-04-12: 收到 52 RTC（20+12+20），PR #2207 合并当天
  - 2026-05-03: 收到 187 RTC（最大一笔，可能是 #5154 的 160 RTC + 奖励）
  - 所有交易来源均为 "founder_team_bounty" 或 "founder_dev_fund"

**结论**: 
- #5154 (160 RTC) 已确认支付 ✓
- Scottcjn 的 "duplicate" 说法成立但付款真实 ✓
- **无需反驳** — 付款已实际到账
...
- **查询结果**: 0.0 RTC ❌
- **Scottcjn 钱包余额**: 120.0 RTC (miner_id: Scottcjn) — 他有钱但不付款

**2026-06-10 最终催款结果**：
- #5154 (160 RTC): 已发最终催款，指出 Scottcjn 钱包有 120 RTC
- #9357 (15 RTC): 已发最终催款
- #9358 (5 RTC): 已发最终催款
- **总计**: 180 RTC 仍未到账，等待回应

**教训**：
1. Scottcjn 钱包有钱但不付款，可能存在系统问题或故意拖延
2. 即使验证过付款历史，仍可能发生变化 (余额归零)
3. 必须进行定期余额验证 (建议每周至少 1 次)
4. 催款后 7 天无回应 → 标记为放弃，不要无限期等待

**新规则**：
- **每周余额验证** — 对 Scottcjn 这类高信誉维护者也要定期验证
- **余额归零预警** — 如果余额从>0 变为 0，立即停止该仓库的所有任务
- **最终催款原则** — 最多 3 轮催款，之后标记为放弃
...
#### RustChain 付款确认
- **#8022** (35 RTC) 已合并 + 全额到账
- 作者：`dtopenclaw`（非 `zhaog100`）
- RustChain 从 P3❌ 升级为 P2✅

#### secondary rate limit 经验
- GitHub HTTP 403 是 secondary rate limit（非永久封禁）
- 通常等待 10-15 分钟即可恢复
- 避免短时间内大量 API 调用

#### 平台评估更新
- UbiquityOS: 唯一已验证平台，付款记录确认
- RustChain: 新增付款确认（#8022）
- homelab-stack: 死亡，放弃
- claude-builders-bounty: 待观察
