...
SWITCH_THRESHOLD=$(cfg 'thresholds.auto_switch' CTX_SWITCH_THRESHOLD 70)
...
COOLDOWN_SECONDS=$(cfg 'notifications.cooldown_seconds' CTX_COOLDOWN 3600)
...
FEISHU_TARGET=$(cfg 'notifications.feishu_target' CTX_FEISHU_TARGET "")
QQ_CHANNEL=$(cfg 'notifications.qq_channel' CTX_QQ_CHANNEL "qqbot")
QQ_ACCOUNT=$(cfg 'notifications.qq_account' CTX_QQ_ACCOUNT "default")
QQ_TARGET=$(cfg 'notifications.qq_target' CTX_QQ_TARGET "")

# Default notification channel: use qqbot if QQ_TARGET is set, otherwise feishu
NOTIFY_CHANNEL=${QQ_CHANNEL:-$FEISHU_CHANNEL}
NOTIFY_ACCOUNT=${QQ_ACCOUNT:-$FEISHU_ACCOUNT}
NOTIFY_TARGET=${QQ_TARGET:-$FEISHU_TARGET}
