...
get_fallback_chain() {
    ...
    # 默认 fallback: glm-5 → qwen3.5-plus → kimi-k2.5
    case "$current_model" in
        *glm-5*)        echo -e "qwen/qwen3.5-plus\nbailian/kimi-k2.5" ;;
        *qwen3.5*)      echo -e "bailian/kimi-k2.5\nzai/glm-5" ;;
        *kimi*)         echo -e "qwen/qwen3.5-plus\nzai/glm-5" ;;
        *)              echo -e "zai/glm-5\nqwen/qwen3.5-plus" ;;
    esac
}
...
case "${1:-}" in
    check)
        ...
        init_fallback_state
        ERROR_MSG="${2:-}"
        CURRENT_MODEL="${3:-$(jq -r '.current_model // "zai/glm-5"' "$FALLBACK_STATE" 2>/dev/null)}"
